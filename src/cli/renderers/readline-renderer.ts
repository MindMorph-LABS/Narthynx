    ...
    help(): void {
      io.writeOut(
        [
          ...
          "/memory                         List pending proposals + approved memory",
          "/memory approve|reject <id>     Approve or reject a proposal",
          "/memory conflicts               Open conflicts + heuristic overlap scan",
          "/memory diff <idA> <idB>       Compare latest revisions",
          "",
          "Bounded subagents (Frontier F20)",
          "/subagents list | inspect …      Profile registry from .narthynx/subagents.yaml",
          "/subagents run <profile> [id] …  Run verifier / planner / safety / critic",
          "/verify [mission-id]             Verifier subagent (report + graph checks)",
          "/critique [id] [--tool …]        Critic: verifier + optional safety pass",
          "",
          "Session",
          "/help                         This help",
          "/exit                         Leave interactive shell",
          "",
          "Context flags (Frontier kernel)",
          "/context [mission-id]",
          "/context why [mission-id]        Last packet manifest (dry-run fallback)",
          "/context diet [mission-id]       context-diet.yaml + manifest dry-run",
          "/context --note <text>",
